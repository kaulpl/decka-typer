...
class DT_Submission {
    ...
    private static function save_payload(array $body, int $uid, string $transport): WP_REST_Response|WP_Error {
        ...
        if ($wpdb->query($lockSql) !== 1) {
            ...
            return new WP_Error('save_lock_failed', 'Typy zostały przygotowane, ale nie udało się zatwierdzić kuponu. Spróbuj ponownie.', ['status'=>500]);
        }

        self::safe_log('round_submitted', 'Zapisano nieedytowalny kupon kolejki.', [
            'round_id'=>$roundId,
