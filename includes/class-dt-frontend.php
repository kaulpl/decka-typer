...
class DT_Frontend {
    ...
    public static function shortcode(): string {
        ...
        self::hero($settings);
        echo '<span id="dt-ad-h1-anchor" class="dt-ad-h1-anchor" aria-hidden="true"></span>';
    ...
    private static function app_shell(): void {
        ...
        echo '<dialog id="dt-submit-modal" class="dt-front-modal"><div class="dt-front-modal-body"><button class="dt-front-modal-x" type="button" data-modal-close aria-label="Zamknij">×</button><div class="dt-modal-icon">' . self::icon('lock') . '</div><span class="dt-front-kicker">ZAPIS WYBRANYCH MECZÓW</span><h2>Zapisać typy?</h2><p>Zapisane spotkania zostaną zatwierdzone osobno. <strong>Każdy kolejny mecz możesz wytypować później</strong>, najpóźniej przed jego rozpoczęciem.</p><div class="dt-modal-actions"><button type="button" class="dt-modal-cancel" data-modal-close>Wróć</button><button type="button" class="dt-modal-confirm" id="dt-confirm-submit">Tak, zapisz wybrane</button></div></div></dialog>';
        echo '<dialog id="dt-artur-ai-modal" class="dt-artur-ai-modal" aria-labelledby="dt-artur-ai-title"><div class="dt-artur-ai-card"><button type="button" class="dt-artur-ai-close" aria-label="Zamknij">×</button><header><img src="'.esc_url(DT_URL.'assets/img/artur-bot/02-myslenie.webp').'" alt="Artur"><div><span>KOŁO RATUNKOWE</span><h2 id="dt-artur-ai-title">Zapytaj Artura</h2><p id="dt-artur-ai-match"></p></div></header><div class="dt-artur-ai-status" id="dt-artur-ai-status" role="status" aria-live="polite">Sprawdzam dostępność…</div><div class="dt-artur-ai-history" id="dt-artur-ai-history" aria-live="polite"></div><div class="dt-artur-ai-prompts" id="dt-artur-ai-prompts"></div><form id="dt-artur-ai-form"><label for="dt-artur-ai-question">Twoje pytanie o ten mecz</label><textarea id="dt-artur-ai-question" maxlength="300" rows="3" required placeholder="Np. która drużyna jest ostatnio w lepszej formie?"></textarea><button type="submit">Zapytaj Artura</button></form><small>Podpowiedź Artura AI nie jest gwarancją trafionego typu, a wszelkie dane dotyczące Twojego profilu anonimizujemy, zanim Artur AI je otrzyma.</small></div></dialog>';
    }
