...
class DT_Admin {
    public static function register(): void {
        ...
        foreach ([
            ...
            'open_round','close_round','reset_typer_data','save_avatar','toggle_expert','update_feedback','test_notifications','save_notification_templates'
        ] as $action) {
    ...
    private static function prediction_dashboard_item(string $label, float $percent, int $value, int $total, string $description, string $tone): void {
        $percentLabel = number_format_i18n($percent, 1) . '%';
    ...
    public static function notifications(): void {
        ...
        self::shell('Powiadomienia','Historia wysłanych i nieudanych przypomnień Web Push/PWA (historia zawiera także dawne wiadomości e-mail). Łącznie: '.number_format_i18n($total).'.');
        self::notification_templates_form();
        echo '<section class="dt-card"><table class="widefat dt-table"><thead><tr><th>Data</th><th>Użytkownik</th><th>Kanał</th><th>Zdarzenie</th><th>Wiadomość</th><th>Status</th><th>Odpowiedź dostawcy</th></tr></thead><tbody>';
        foreach ((array)$rows as $row) echo '<tr><td>'.esc_html(self::date_pl((string)$row->created_at)).'</td><td><strong>'.esc_html((string)($row->display_name?:'Użytkownik #'.$row->user_id)).'</strong><small class="dt-muted">'.esc_html((string)$row->user_email).'</small></td><td>'.self::badge(strtoupper((string)$row->channel),'blue').'</td><td>'.esc_html((string)$row->event_type).'</td><td><strong>'.esc_html((string)$row->title).'</strong><small class="dt-muted">'.esc_html((string)$row->message).'</small></td><td>'.self::badge((string)$row->status,$row->status==='sent'?'green':($row->status==='failed'?'red':'orange')).'</td><td><code style="white-space:pre-wrap;word-break:break-word">'.esc_html((string)$row->provider_response).'</code></td></tr>';
        if (!$rows) echo '<tr><td colspan="7" class="dt-empty">Historia powiadomień jest jeszcze pusta.</td></tr>';
        echo '</tbody></table></section>';
    ...
    private static function notification_templates_form(): void {
        echo '<section class="dt-card"><h2>Szablony powiadomień</h2><p>Tytuły otrzymują automatycznie prefiks <strong>TypujKosza.pl - </strong>. Szablony przypomnień i zmian terminów są wspólne dla Push i wiadomości w aplikacji. Zmienne w nawiasach klamrowych uzupełniamy danymi meczu i użytkownika. Daty podajemy w strefie czasowej WordPressa. Zmiana tekstu nie zmienia harmonogramu wysyłki.</p><p>Powitanie dotyczy pierwszej subskrypcji Push. W OneSignal wybierz integrację Custom Code, aby używać powitania z tych ustawień. Nazwę źródła powiadomienia może dodatkowo wyświetlać system telefonu.</p><form method="post" action="'.esc_url(admin_url('admin-post.php')).'"><input type="hidden" name="action" value="dt_save_notification_templates">';
        wp_nonce_field('dt_save_notification_templates');
        foreach (DT_Notifications::templates() as $key=>$template) {
            echo '<fieldset class="dt-section"><legend><strong>'.esc_html($template['label']).'</strong></legend><p><label for="dt-template-'.$key.'-title">Tytuł (bez prefiksu)</label><br><input class="large-text" id="dt-template-'.$key.'-title" name="templates['.$key.'][title]" value="'.esc_attr($template['title']).'" required></p><p><label for="dt-template-'.$key.'-message">Treść</label><br><textarea class="large-text" rows="3" id="dt-template-'.$key.'-message" name="templates['.$key.'][message]" required>'.esc_textarea($template['message']).'</textarea></p><p class="description">Dostępne zmienne: ';
            echo $template['variables']?esc_html(implode(', ',array_map(static fn($v)=>'{'.$v.'}',$template['variables']))):'brak — tekst stały';
            echo '</p></fieldset>';
        }
        echo '<p class="description">{liga}: PLK/1LM/2LM; {grupa}: np. grupa A lub pusty tekst; {kolejka}: nazwa kolejki; {mecz}: gospodarz – gość; {termin}, {stary_termin}, {nowy_termin}: data i godzina lub „termin do ustalenia”; {pozostalo}: liczba meczów bez typu dla odbiorcy.</p><button class="button button-primary" type="submit">Zapisz szablony powiadomień</button></form></section>';
    }

    public static function save_notification_templates(): void {
        self::guard('dt_save_notification_templates');
        $result=DT_Notifications::save_templates((array)wp_unslash($_POST['templates']??[]));
        self::redirect('decka-typer-notifications',is_wp_error($result)?$result->get_error_message():'Zapisano szablony powiadomień.',is_wp_error($result)?'error':'success');
    }
