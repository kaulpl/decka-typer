(()=>{
  ...
  const matchCard=m=>{
    ...
    return `<article class="dt-match ${decka?'is-decka':''} ${canPick?'':'is-locked'}" data-match-card="${m.id}">
      ...
      </div>${result}<details class="dt-match-more"><summary><span aria-hidden="true">▾</span> Zobacz statystyki <span aria-hidden="true">▾</span></summary><div class="dt-match-insights-grid">${teamInsights(m.home_name,m.home_insights,'home')}${teamInsights(m.away_name,m.away_insights,'away')}</div></details>${cfg.arturAiEnabled&&canPick?`<button type="button" class="dt-artur-ai-open" data-artur-ai data-round="${state.round.id}" data-match="${m.id}" data-home="${esc(m.home_name)}" data-away="${esc(m.away_name)}"><span aria-hidden="true">🛟</span><strong>Koło ratunkowe Artura</strong><small>Jedno na kolejkę</small></button>`:''}</article>`;
  };
